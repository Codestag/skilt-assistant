...
function stag_add_meta_box( $meta_box ) {
	...
	if ( is_array( $page ) ) {
		foreach ( $page as $type ) {
			add_meta_box( $meta_box['id'], $meta_box['title'], $callback, $type, $meta_box['context'], $meta_box['priority'], $meta_box );
		}
	} else {
...
function stag_create_meta_box( $post, $meta_box ) {
	...
	foreach ( $meta_box['fields'] as $field ) {
		...
		echo '<tr class="type-' . $field['type'] . '"><th><label for="' . $field['id'] . '"><strong>' . $field['name'] . '</strong>
		...
		switch ( $field['type'] ) {
			...
			case 'hidden':
				echo '<td><input type="hidden" name="stag_meta[' . $field['id'] . ']" id="' . $field['id'] . '" value="' . ( $meta ? $meta : $field['std'] ) . '" size="30" /></td>';
				break;
			...
			case 'images':
				...
				$thumbs_output = '';
				$button_text   = ( $meta ) ? __( 'Edit Gallery', 'skilt-assistant' ) : $field['std'];
				if ( $meta ) {
					$field['std']  = __( 'Edit Gallery', 'skilt-assistant' );
					$thumbs        = explode( ',', $meta );
					$thumbs_output = '';
